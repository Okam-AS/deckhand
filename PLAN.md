...
- `deckhand secret set|rm typesafe` — the TypeSafe key `navigate` needs, from stdin or
  ...
  consent for `navigate` to send that app's screens to TypeSafe; `deckhand navigate status`, and a
  `navigate egress` line in `deckhand doctor`, say which apps it is on for and whether a key is set.
...
5. **Secrets**: app secrets never through MCP or the viewer. Two write channels only:
   ...
   **Egress — the one path by which app content leaves this machine for a third party.**
   ...
   - **Every request is audited** as a `navigate:egress` line in `audit.jsonl`: app, preview,
     ...
     request goes out.
   - **Visible.** `deckhand doctor` prints a `navigate egress` line — which apps, and whether a
     key is set — as a warning while screens can leave the machine.
