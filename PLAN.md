...
| Navigate: a decider inside deckhand, opt-in per app | **Accepted (2026-09-23).** `navigate` (§6) runs `describe` → one TypeSafe Jev decision → `ui` on the server. It is not the "hand the drive loop to a cheaper model" advice §6 records as removed on 2026-08-02: that added agent round trips (66 MCP calls, 583s, ~5% of it deckhand) and let a weaker agent author confident wrong findings. Here the caller makes ONE call; the decider picks from a closed list code built from the tree — tap an on-screen element's centre, back, scroll, type a value the caller named, `done`, `stuck`, never `openUrl`, free text or a coordinate of its own — and it can author no finding, so judging the app stays with the caller. Without a key the tool does not exist at all. It is also the **first path by which app content leaves this machine for a third party**, so it needs both a key and the operator's consent PER APP (`deckhand navigate enable <app>`, `navigateEgress` in apps.yaml, never settable over MCP), sends only minimised, masked text, and audits every request (§11 item 5). **Until a DPA with zero data retention is signed with TypeSafe, enable it only for apps showing test data and test accounts.** |
...
apps:
  ...
  - id: my-app-rn                 # migration target (§6 migration features, 2026-07-18)
    ...
    navigateEgress: true          # consent for `navigate` to send this app's screen labels to TypeSafe (§11 item 5);
                                  # set by `deckhand navigate enable`, never over MCP
```
...
- `deckhand secret set|rm typesafe` — the TypeSafe key `navigate` needs, from stdin or
  `TYPESAFE_API_KEY`, never argv. `deckhand navigate enable|disable <appId>` — the per-app
  consent for `navigate` to send that app's screens to TypeSafe; `deckhand navigate status`, and —
  once a key file exists, never before — a `navigate egress` line in `deckhand doctor`, say which
  apps it is on for.
...
5. **Secrets**: app secrets never through MCP or the viewer. Two write channels only:
   ...
   **Egress — the one path by which app content leaves this machine for a third party.**
   ...
   - **Consent per app.** A key alone sends nothing: the app must carry `navigateEgress: true`,
     which only `deckhand navigate enable <app>` on the machine writes. `add_app` cannot set it,
     a pane (which runs under a synthetic id) is never covered, and removing the flag takes
   ...
   - **Every request is audited** as a `navigate:egress` line in `audit.jsonl`: app, preview,
     ...
     request goes out.
   - **Visible.** Once a key file exists, `deckhand doctor` prints a `navigate egress` line — which
     apps, or that the key cannot be read — as a warning while screens can leave the machine. With
     no key file there is no line and no tool.
   - **Contract.** TypeSafe states that requests are not used for training and offers zero data
     ...
     one is, and until where the data is processed is settled (a GDPR transfer question), enable
     it only for apps that show test data and test accounts.
