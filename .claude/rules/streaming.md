...
- **A device-side recorder has NO env marker, so the boot sweep proves ownership four other ways.** `sweepDeviceRecorders` kills a `screenrecord` only when all four hold: the serial is an emulator (a physical device is borrowed hardware we never stream, so a recorder there is always someone else's), `adb emu avd name` answers with a `deckhand_` prefix, neither the serial nor its AVD name is in `keep` **and no helper of ours holds the serial** — that second half is not a re-reading of `keep`, it is what catches an attach landing DURING the sweep, later than the keep-set snapshot was taken — and no host-side `adb -s <serial> exec-out screenrecord` is alive. That last gate is not belt-and-braces: `cli/doctor.ts` calls `reapOrphans()` from a SECOND process with an empty keep-set, and without a cross-process owner signal the sweep would kill the running server's video. Every failure path kills nothing — a failed `adb devices` means "no orphans", a failed `ps` abandons the pass as "unknown owners".
