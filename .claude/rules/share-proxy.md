...
- **A PIN never rides the URL.** Unlock is a POST to `/__deck/unlock` (or `/s/:shareId/unlock`), and the cookie it sets is the only credential the proxy reads. An earlier design had the WS upgrade accept `?password=` and strip it before proxying. It was a SPEC, never code — `git log -G'?password='` finds it in PLAN.md and in this file, and nowhere under `server/` or `viewer/` — so "the stripping worked" was never a thing anyone could have observed, and this rule used to claim it. It was the wrong shape whether it would have worked or not: a share URL gets pasted into chat, bookmarks and screenshots, and a query parameter is part of the URL. There is nothing to strip if nothing is accepted.
