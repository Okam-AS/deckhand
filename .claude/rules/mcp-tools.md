...
- **Wrap the handler in `audited()`.** PLAN §11 item 2 is a JSONL audit of every call; a tool added without it is invisible to the trail and nothing else notices. → `invariants.test.ts` "audits every registered MCP tool"
