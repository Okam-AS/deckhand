...
async function cmdSecretSetTypesafe(): Promise<void> {
  ...
  console.log(`stored the TypeSafe key at ${paths.typesafeKey()} (0600, never exposed via MCP). navigate now appears to agents and sends the screen labels of every app it drives to TypeSafe (api.typesafe.ai) — test data and test accounts only. Undo: deckhand secret rm typesafe`);
}
